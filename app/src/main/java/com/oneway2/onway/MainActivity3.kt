...
class MainActivity3 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        navigation_view3.setNavigationItemSelectedListener{
            when (it.itemId) {
                R.id.nav_home -> {
        ...
        drawer_layout3.addDrawerListener(drawerToggle)
        drawerToggle.syncState()

    }

    override fun onBackPressed() {
        if (drawer_layout3.isDrawerOpen(GravityCompat.START)) {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        fun loadURL() {
            webview2.loadUrl("http://onway.ps/my_orders")
            webview2.settings.javaScriptEnabled = true
            webview2.settings.loadsImagesAutomatically = true
